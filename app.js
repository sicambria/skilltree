...
app.post('/registration', async function (req, res) {
	...
	if (!user) { // if new user
		...
		var focusAreaTrees = await Tree.find({ focusArea: req.body.focusArea }, { _id: 0, name: 1 });
...
protectedRoute.get('/userdata', async function (req, res) {
	try {
		...
		if (!userdata) {
			...
			if (user.mainTree == undefined) { // first login
				var trees = await Tree.find({});
				user.allTreeNames = trees;
			}

			if (user.admin) {
				var trees = await ApprovableTree.find({});
				var skills = await ApprovableSkill.find({});
				var trainings = await ApprovableTraining.find({});
...
protectedRoute.get('/offers', async function (req, res) {
	try {
...
// getting the skilldata of a skillname (used for offers)
protectedRoute.get('/skillsforapproval', async function (req, res) {
	try {
		const skillsforapproval = await ApprovableSkill.find({});
...
protectedRoute.post('/getPublicSkillData', async function (req, res) {
	...
	for (var s = 0; s < foundSkills.length; s++) {
		foundSkills[s].users = [];
		var foundUsers = await User.find({}, 'username skills');
		outUsers = [];
		for (var i = 0; i < foundUsers.length; i++) {
...
protectedRoute.post('/addTreeToUser', async function (req, res) {
	var data = req.body;
	var user = await findUser(req.decoded.username);
	var tree = await Tree.findOne({ "name": data.value });
	...
	if (skill !== undefined) {
		...
		if (req.body.requestType == "intermediate") {
			if (skill.intermediateRequests.find(obj => obj.username == user.username) == undefined) {
				skill.intermediateRequests.push({
					username: user.username,
					achievedPoint: userskill.achievedPoint,
					email: user.email
				});
...
adminRoute.post('/approveskill', async function (req, res) {
	...
	var approvecollection = await ApprovableSkill.find({});
	...
	else  //If its not, add to the database
	{
		...
		for (var i = 0; i < lastdependency.parents.length; i++) {
			var lastdependencyParent = await Skill.find({ name: lastdependency.parents[i] });
...
adminRoute.post('/approvetree', async function (req, res) {
	...
	if (globalTree == undefined) {
		...
		await ApprovableTree.deleteMany({ // delete ALL trees from approve with this name
			name: data.name
		});
	}
});
...
adminRoute.post('/approvetraining', async function (req, res) {
	...
	if (globalSkill.trainings.find(obj => obj.name == data.name) == undefined) {
		...
		await ApprovableTraining.deleteMany({ // delete ALL trainings for the skill from approve with this name
			skillName: data.skillName,
			name: data.name
		});
	}
